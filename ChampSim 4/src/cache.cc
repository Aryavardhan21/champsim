...
#include"user_para.h"
  ...
  int SIZE = 2048;
      std::vector<std::vector<uint64_t>> set_map(2048, std::vector<uint64_t>(SIZE));
      std::vector<uint32_t> remappingtoset(SIZE);
       std::vector<uint32_t> llc_set_accesses(SIZE);
     std::vector<uint32_t> llc_set_evictions(SIZE);
      std::vector<std::vector<uint32_t>> block_restore(SIZE, std::vector<uint32_t>(SIZE));
      std::vector<uint64_t> no_of_block_restores(SIZE);
      std::vector<std::pair<uint64_t, float>> degree_of_usage(SIZE);
     std:: uint64_t llc_total_accesses;
...
void CACHE::handle_fill(){
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if(cache_type==IS_LLC){
          set = LLC_get_set(MSHR.entry[mshr_index].address);
        ...
        if (cache_type == IS_LLC) {
            way = llc_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
        }
        else
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        uint32_t set;
        if(cache_type==IS_LLC){
          set = get_the_set(WQ.entry[index].address);
        }
        else{
        ...
        int way;
        if(cache_type==IS_LLC){
          way = LLC_check_hit(&WQ.entry[index],set);
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                uint32_t set, way;
                if(cache_type==IS_LLC){
                    set = LLC_get_set(WQ.entry[index].address);
                }
                else{
                    set = get_set(WQ.entry[index].address);
                }
                if (cache_type == IS_LLC) {
                    way = llc_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type);
                }
                else
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (read_cpu == NUM_CPUS)
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            uint32_t set ;
            if(cache_type==IS_LLC){
          set = get_the_set(RQ.entry[index].address);
        }
        else{
          set = get_set(RQ.entry[index].address);
            ...
            int way;
        if(cache_type==IS_LLC){
          way = LLC_check_hit(&RQ.entry[index],set);
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (prefetch_cpu == NUM_CPUS)
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
            ...
            uint32_t set ;
            if(cache_type==IS_LLC){
          set = get_the_set(PQ.entry[index].address);
        }
        else{
          set = get_set(PQ.entry[index].address);
            ...
            int way;
        if(cache_type==IS_LLC){
          way = LLC_check_hit(&PQ.entry[index],set);
...
int CACHE::check_hit(PACKET *packet,uint32_t set1)
{
  uint32_t set;
  if(cache_type==IS_LLC){
      set=set1;
...
uint32_t CACHE::LLC_get_set(uint64_t address) {
    uint32_t set = (uint32_t)(address & ((1 << lg2(NUM_SET)) - 1));
    llc_set_accesses[set]++;
    llc_total_accesses++;
    int i = 0, j = 0;

    if (llc_set_evictions[set] > 0) {
        for (i = 0; i < LLC_SET; i++) {
            for (j = 0; j < LLC_WAY; j++) {
                if (block[i][j].valid == 0) {
                    if (no_of_block_restores[set] < LLC_SET) {
                        block_restore[set][no_of_block_restores[set]] = address;
                        no_of_block_restores[set]++;
                    }
                    return i;
                }
            }
        ...
        if (i == LLC_SET) {
            compute_degree_of_set_usage();
            for (int k = 0; k < LLC_SET; k++) {
                if (set == degree_of_usage[k].first) {
                    if (remappingtoset[set] < LLC_SET) {
                        set_map[set][remappingtoset[set]] = degree_of_usage[LLC_SET - k - 1].first;
                        remappingtoset[set]++;
                        set = degree_of_usage[LLC_SET - k - 1].first;
                    }
                    break;
                }
            }
...
bool compareSecond(const pair<uint64_t, float>& a, const pair<uint64_t, float>& b) {
    return a.second > b.second;
}

void CACHE::compute_degree_of_set_usage() {
    for (int i = 0; i < LLC_SET; i++) {
        degree_of_usage[i].first = i;
        degree_of_usage[i].second = (llc_set_accesses[i] > 0) ? (float)llc_set_evictions[i] / llc_set_accesses[i] : 0.0f;
    }
    sort(degree_of_usage.begin(), degree_of_usage.end(), compareSecond);
}

uint32_t CACHE::llc_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t ip, uint64_t full_addr, uint32_t type) {
    int i = 0, j = 0;
    ...
    for (i = 0; i < LLC_SET; i++) {
        for (j = 0; j < LLC_WAY; j++) {
            if (block[i][j].valid == 0) {
                if (no_of_block_restores[set] < LLC_SET) {
                    block_restore[set][no_of_block_restores[set]] = block[set][way].address;
                    no_of_block_restores[set]++;
                }
                copy_block(set, way, i, j);
                break;
            }
        }
        if (j < LLC_WAY) break;
    }
    llc_set_evictions[set]++;

    return way;
}

int CACHE::LLC_check_hit(PACKET* packet,uint32_t set) {
    uint32_t set1 = set;

    // Check if it's time to reset the cache state (every 100000 accesses)
  
    int match_way = check_hit(packet,set);

    // Iterate over possible remapped sets for the given set index
    for (int i = 0; i < remappingtoset[set1]; i++) {
        uint64_t set = set_map[set1][i];
        if (set < NUM_SET) {
            // Check each way within the set
            for (uint32_t way = 0; way < NUM_WAY; way++) {
                if (block[set][way].valid && (block[set][way].tag == packet->address)) {
                    // Cache hit: update match_way and print debug message if warmup is complete
                    match_way = way;
                    ...
                    if (warmup_complete[packet->cpu]) {
                        ...
                        cout << " full_addr: " << packet->full_addr << " tag: " << block[set][way].tag << " data: " << block[set][way].data << dec;
                        cout << " set: " << set << " way: " << way << " lru: " << block[set][way].lru;
                        cout << " event: " << packet->event_cycle << " cycle: " << current_core_cycle[packet->cpu] << endl;
                    }

                    return match_way; // Return the matched way
                }
            }
        } else {
            // Handle invalid set index (out of range)
            cerr << "[" << NAME << "_ERROR] " << __func__ << " invalid set index: " << set << " NUM_SET: " << NUM_SET;
            cerr << " address: " << hex << packet->address << " full_addr: " << packet->full_addr << dec;
            cerr << " event: " << packet->event_cycle << endl;
        }
    ...
    // Check for block restores if no hit found
    if (no_of_block_restores[set1] > 0) {
        for (int j = 0; j < no_of_block_restores[set1]; j++) {
            if (block_restore[set1][j] == packet->address) {
                // Check each way within the set for a restore block
                for (uint32_t way = 0; way < NUM_WAY; way++) {
                    if (block[set1][way].valid && (block[set1][way].tag == packet->address)) {
                        // Cache hit on restore: update match_way and print debug message if warmup is complete
                        match_way = way;

                        if (warmup_complete[packet->cpu]) {
                            cout << "[" << NAME << "] " << __func__ << " instr_id: " << packet->instr_id << " type: " << +packet->type << hex << " addr: " << packet->address;
                            cout << " full_addr: " << packet->full_addr << " tag: " << block[set1][way].tag << " data: " << block[set1][way].data << dec;
                            cout << " set: " << set1 << " way: " << way << " lru: " << block[set1][way].lru;
                            cout << " event: " << packet->event_cycle << " cycle: " << current_core_cycle[packet->cpu] << endl;
                        }
    ...
    // If no hit found, return match_way which is -1
    return match_way;
}
...
void CACHE::reset() {
    for (int i = 0; i < LLC_SET; ++i) {
        ...
        for (int j = 0; j < LLC_SET; ++j) {
            block_restore[i][j] = 0;
            no_of_block_restores[i] = 0;
        }
    }
}

// Function to copy data from block[set][way] to block[i][j]
void CACHE :: copy_block(int set, int way, int i, int j) {
    // Copy valid, prefetch, dirty, and used flags
    block[i][j].valid = block[set][way].valid;
    block[i][j].prefetch = block[set][way].prefetch;
    block[i][j].dirty = block[set][way].dirty;
    block[i][j].used = block[set][way].used;

    // Copy delta, depth, signature, and confidence
    block[i][j].delta = block[set][way].delta;
    block[i][j].depth = block[set][way].depth;
    block[i][j].signature = block[set][way].signature;
    block[i][j].confidence = block[set][way].confidence;

    // Copy address, full_addr, tag, data, ip, cpu, and instr_id
    block[i][j].address = block[set][way].address;
    block[i][j].full_addr = block[set][way].full_addr;
    block[i][j].tag = block[set][way].tag;
    block[i][j].data = block[set][way].data;
    block[i][j].ip = block[set][way].ip;
    block[i][j].cpu = block[set][way].cpu;
    block[i][j].instr_id = block[set][way].instr_id;

    // Copy lru and set it to j
    block[i][j].lru = j;
}
...
uint32_t CACHE :: get_the_set(uint64_t address){
  for(int i=0;i<LLC_SET;i++){
    for(int j=0;j<LLC_WAY;j++){
      if(block[i][j].valid && block[i][j].tag==address)
        return i;
