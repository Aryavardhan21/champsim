...
class CACHE : public MEMORY {
  public:
    ...
    uint64_t sim_access[NUM_CPUS][NUM_TYPES],
      ...
      uint64_t  llc_eviction[LLC_SET];
     uint64_t llc_accesses[LLC_SET];
      uint64_t llc_total_accesses;
      uint64_t set_map[LLC_SET][LLC_SET];
      uint32_t mappingofset[LLC_SET];
    ...
    vector<float> degreeofset_usage();

    uint32_t get_occupancy(uint8_t queue_type, uint64_t address),
             get_size(uint8_t queue_type, uint64_t address),
            get_the_set(uint64_t address);
...
uint32_t LLC_get_set(uint64_t address);
void  compute_degree_of_set_usage();
uint32_t llc_victim (uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
int LLC_check_hit(PACKET *packet,uint32_t set);
void reset() ;

void copy_block(int set, int way, int i, int j);

    uint32_t get_set(uint64_t address),
            llc_get_set(uint64_t address),
